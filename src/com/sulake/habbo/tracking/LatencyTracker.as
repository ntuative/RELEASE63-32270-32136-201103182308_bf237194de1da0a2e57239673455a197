package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      {
         if(this.var_178 == null || this._communication == null || this._connection == null)
         {
            return;
         }
         this.var_1561 = int(this.var_178.getKey("latencytest.interval"));
         this.var_1559 = int(this.var_178.getKey("latencytest.report.index"));
         this.var_2075 = int(this.var_178.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1560 > this.var_1561)
         {
            this.testLatency();
         }
      }
      ...
      {
         this.var_1560 = getTimer();
         this.var_562.add(this.var_1303,this.var_1560);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1303));
         ++this.var_1303;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_562 == null || this.var_215 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_562.getValue(_loc2_.requestId);
         this.var_562.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_215.push(_loc4_);
         if(this.var_215.length == this.var_1559 && this.var_1559 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1558) > this.var_2075 || this.var_1558 == 0)
            {
               this.var_1558 = _loc9_;
