package com.sulake.habbo.help
{
   ...
   public class WelcomeScreenController implements IUpdateReceiver
   {
      ...
      public function WelcomeScreenController(param1:HabboHelp, param2:IHabboWindowManager, param3:IHabboConfigurationManager)
      {
         ...
         this._config = param3;
         this.var_130.toolbar.events.addEventListener(HabboToolbarEvent.const_38,this.onToolbarClicked);
         this.var_130.toolbar.events.addEventListener(HabboToolbarEvent.const_1034,this.onToolbarResized);
         var _loc4_:Boolean = Boolean(parseInt(this._config.getKey("new.identity","0")));
         this.var_1638 = Boolean(parseInt(this._config.getKey("welcome.screen.enabled","0"))) && _loc4_;
         this.var_2197 = parseInt(this._config.getKey("welcome.screen.delay","5000"));
         this.var_369 = new Point(72,10);
         this.var_199 = new Timer(this.var_2197,1);
      ...
      public function dispose() : void
      {
         ...
         if(this.var_130)
         {
            this.var_130.toolbar.events.removeEventListener(HabboToolbarEvent.const_38,this.onToolbarClicked);
            this.var_130 = null;
         }
      ...
      public function setCategory(param1:String, param2:String = "", param3:String = "") : void
      {
         ...
         {
            this.var_1636 = "welcome.screen.title";
         }
         if(param3.length > 0)
         {
         ...
         {
            this.var_1637 = "welcome.screen.message";
         }
      }
      
      private function onTimerComplete(param1:TimerEvent) : void
      {
         var _loc2_:XmlAsset = this.var_130.assets.getAssetByName("welcome_screen_xml") as XmlAsset;
         ...
         var _loc4_:BitmapDataAsset = this.var_130.assets.getAssetByName("welcome_screen_arrow") as BitmapDataAsset;
         ...
         _loc5_.content.setParamFlag(WindowParam.const_280,false);
         _loc5_.height -= 20;
         this._window.findChildByName("close").addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onMouseClick);
         this.var_130.registerUpdateReceiver(this,10);
         this.var_1638 = false;
         this.var_369.y = this.var_130.toolbar.getIconLocation(this.var_1342) - this._window.height / 2;
         this._window.y = this.var_369.y;
         this._window.findChildByName("title").caption = "${" + this.var_1636 + "}";
         this._window.findChildByName("text").caption = "${" + this.var_1637 + "}";
         this.highlightToolbarIcon(true);
      }
      
      private function highlightToolbarIcon(param1:Boolean) : void
      {
         var _loc2_:HabboToolbarSetIconEvent = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_92,this.var_1342);
         _loc2_.iconState = !!param1 ? "highlight_blue" : "0";
         this.var_130.toolbar.events.dispatchEvent(_loc2_);
      }
      ...
      private function onToolbarResized(param1:HabboToolbarEvent) : void
      {
         ...
         if(this.var_130)
         {
            this.var_130.removeUpdateReceiver(this);
            this.var_130.registerUpdateReceiver(this,10);
         }
         this.var_369.y = this.var_130.toolbar.getIconLocation(this.var_1342) - this._window.height / 2;
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         var _loc2_:Number = Point.distance(this._window.rectangle.topLeft,this.var_369);
         if(_loc2_ > 5)
         {
            _loc3_ = Point.interpolate(this._window.rectangle.topLeft,this.var_369,0.5);
            this._window.x = _loc3_.x;
            this._window.y = _loc3_.y;
         }
         else
         {
            this._window.x = this.var_369.x;
            this._window.y = this.var_369.y;
            this.var_130.removeUpdateReceiver(this);
         }
      }
