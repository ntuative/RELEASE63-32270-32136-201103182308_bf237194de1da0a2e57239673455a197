package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2050:int = 100;
      
      private const const_1522:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_976 = new Timer(this.const_2050,1);
         this.var_976.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_438)
         {
            this.var_438.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_438.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_438)
         {
            this.var_438.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_438.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_975);
            if(this.var_1467.indexOf("?") > 0)
            {
               _loc2_ = this.var_1467 + "&retry=" + this.var_975;
            }
            else
            {
               _loc2_ = this.var_1467 + "?retry=" + this.var_975;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2611,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1198[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1198[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_489[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_489[_loc3_] = _loc6_;
            }
            this.var_977[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_977)
         {
            _loc7_ = true;
            _loc4_ = this.var_977[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_489[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_977[_loc3_];
         }
         while(_loc6_ < this.var_779.length)
         {
            _loc5_ = this.var_779[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_779.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_976.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_778.length > 0 && this.var_779.length < this.const_1522)
         {
            _loc1_ = this.var_778[0];
            _loc1_.startDownloading();
            this.var_779.push(this.var_778.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_778.indexOf(param1) == -1 && this.var_779.indexOf(param1) == -1)
         {
