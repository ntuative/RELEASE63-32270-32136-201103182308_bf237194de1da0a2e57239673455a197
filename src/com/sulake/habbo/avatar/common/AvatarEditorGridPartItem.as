package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_826:IFigurePartSet;
      ...
      private const const_2014:int = 13421772;
      
      private const const_2013:int = 16777215;
      
      private const const_337:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_675 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
         ...
         this.var_451 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_675 != null && !this.var_2001)
            {
               _loc3_ = this.var_675;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_826)
            {
               switch(this.var_826.clubLevel)
               {
                  case HabboClubLevelEnum.const_53:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_749;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_784;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_337[_loc6_] + "_" + FigureData.const_139;
               _loc4_ = this.var_451.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_337.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_337[_loc6_] + "_" + FigureData.const_139;
                  _loc4_ = this.var_451.getAssetByName(_loc3_) as BitmapDataAsset;
         ...
         if(this.var_671 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1999)
               {
                  ...
                  this.var_1999 = _loc9_.content as BitmapData;
               }
