package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class RecyclerPrizesCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_658 = this.recycler.getPrizeTable();
         ...
         if(this.var_658.length > 0 && this.var_658[0].prizes.length > 0)
         {
            this.select(this.var_658[0].prizes[0] as IGridItem);
         }
         return true;
      }
      ...
      {
         var _loc2_:int = 0;
         while(_loc2_ < this.var_658.length)
         {
            this.createLevelItem(this.var_658[_loc2_]);
            _loc2_++;
         }
      }
      ...
      private function createPrizeItem(param1:PrizeContainer, param2:IItemGridWindow) : void
      {
         ...
         var _loc3_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(this.var_1939) as IWindowContainer;
