package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1725:uint = 1;
      
      protected static const const_1724:uint = 2;
      
      protected static const const_1726:uint = 4;
      
      protected static const const_1727:uint = 3;
      
      protected static const name_3:uint = 7;
      
      public static const const_1683:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1059:Boolean = false;
      
      public static const const_1522:int = 2;
      
      private static var var_893:Array = new Array();
      
      private static var var_720:Array = new Array();
      ...
      protected var var_237:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_638:Class;
      
      protected var var_460:uint = 0;
      
      protected var _name:String;
      
      protected var var_1038:Boolean = false;
      ...
      protected var var_2819:String = "";
      
      protected var var_2054:int;
      
      protected var var_1290:int;
      
      protected var var_1289:int;
      
      protected var var_2907:uint = 0;
      
      protected var var_2821:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_43.load(param1.var_237,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1059)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_720[_loc2_] as LibraryLoader;
            ...
            while(var_720.length < const_1522 && false)
            {
               _loc1_ = var_893.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_720.push(_loc1_);
                  _loc1_.var_43.load(_loc1_.var_237,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_237 ? this.var_237.url : null;
      }
      ...
      {
         return !!this.var_1038 ? uint(this.var_1290 - this.var_2054) : uint(getTimer() - this.var_2054);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_237.url);
      ...
      {
         if(this.var_1037 && !_disposed)
         {
            this.var_1037 = false;
            if(!this.var_1038 && this.var_237)
            {
               this.var_1290 = -1;
      ...
      {
         if(!this.var_1038 && !_disposed && this.var_1289 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_237,const_1683 - this.var_1289);
            if(this.var_2821)
            {
               this.var_237.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_237.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2821 = false;
            }
            --this.var_1289;
            this.var_460 = 0;
            this.var_43.load(this.var_237,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_397 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_397 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_397))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_460 = LibraryLoader.name_3;
         ...
         if(this.var_460 == LibraryLoader.const_1727)
         {
         ...
         if(this.var_460 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_638 = this.getDefinition(this._name) as Class;
         if(this.var_638 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_237.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_638.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_397,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
